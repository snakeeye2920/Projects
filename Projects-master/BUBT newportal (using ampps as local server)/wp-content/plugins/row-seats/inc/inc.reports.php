...
if(isset($_REQUEST['ctxnid']))
{
        ...
        if ($results = $wpdb->get_results($sql, ARRAY_A)) {
			...
			$ticketno = $rst_options['rst_ticket_prefix'];
...
$etrafound=mysql_query("select * from rst_bookings where booking_id=".$_REQUEST['bid']);
$asql = $wpdb->get_row($etrafound, ARRAY_A);	
    ...
    <table>
      <tr><td>Booked By</td><td><input type="text" name="b_by" value="<?php echo $value['name'];  ?>"/></td></tr>
       ...
       <tr><td>Ticket No</td><td><input type="text" name="ts_no" value="<?php echo $value['ticket_seat_no'];  ?>"/></td></tr>
		...
		if(isset($extra['customfield']) && $extra['customfield']!=''){
foreach ($extra['customfield'] as $key => $val){
$key_name=str_replace(" ","_",$key);
...
if (isset($_REQUEST['action']) && $_REQUEST['action'] = 'Apply') {
	/*$rst_rpfrom = "";
	if(isset($_POST['rst_rpfrom']))
	$rst_rpfrom = $_POST['rst_rpfrom'];
    $alldata = bookedtickets($_REQUEST['rstfilter'], $rst_rpfrom, $_REQUEST['rst_rpto']);*/
    
	$rstfilter="";
	$rst_rpfrom="";
	$rst_rpto="";
	
	if(isset($_REQUEST['rstfilter']))
		$rstfilter = $_REQUEST['rstfilter'];
	if(isset($_REQUEST['rst_rpfrom']))
		$rst_rpfrom = $_REQUEST['rst_rpfrom'];	
	if(isset($_REQUEST['rst_rpto']))
		$rst_rpto = $_REQUEST['rst_rpto'];						
	
    $alldata = bookedtickets($rstfilter, $rst_rpfrom, $rst_rpto);    
...
if(isset($_REQUEST['bookingid']) && $_REQUEST['bookingid'])
{
    $_SESSION['rstfilter'] = '';
    $_SESSION['rst_rpfrom'] = '';
    $_SESSION['rst_rpto'] = '';
...
<form action="<?php echo get_option('home') ?>/wp-admin/admin.php?page=rst-reports" method="GET" name="filteraction">
    <select name="rstfilter" id="rstfilter">
        ...
        for ($i = 0; $i < count($showdata); $i++) {
            ?>
            <option
                value="<?php if(isset($showdata[$i]['id'])) echo $showdata[$i]['id']; ?>" <?php if (isset($alldata[$i]['id']) && $alldata[$i]['id'] == $filter) echo 'selected'?> ><?php if(isset( $showdata[$i]['show_name'])) echo $showdata[$i]['show_name']; else echo "Unknown show name"; ?></option>
...
<input type="text"
                                                                                                          ...
                                                                                                          id="keywords"
                                                                                                          class="regular-text cpnfrom "
                                                                                                          value="<?php if(isset($_REQUEST['keywords'])) echo $_REQUEST['keywords'];?>"
                                                                                                          size="30"
                                                                                                          style="width: 200px;" />
