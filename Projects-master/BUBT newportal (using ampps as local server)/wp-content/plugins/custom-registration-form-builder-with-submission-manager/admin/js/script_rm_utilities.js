...
function rm_get_help_text_price_field(ftype){
    ...
    switch(ftype)
    {
        case 'fixed':return 'For creating fixed price single product.';
        case 'multisel':return 'Allow user to pick multiple products with individual prices. Price will calculated as cumulative for the selection for products.';
        case 'dropdown':return 'Allows user to pick a single product from multiple products with individual prices.';
        case 'userdef':return 'Allows user to enter his/ her own price for product with the form. Useful for accepting donations etc.';
        default: return 'Define how the product will be priced.';
    }
}
