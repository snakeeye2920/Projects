    ...
    protected function hook_pre_field_addition_to_page($form, $page_no)
    {
        //if (1 == $page_no)
        {
             ...
             */
                ...
                if ($this->service->get_setting('auto_generated_password') != 'yes')
                {
                    $form->addElement(new Element_Password(RM_UI_Strings::get('LABEL_PASSWORD'), "password", array("required" => 1,"placeholder"=>RM_UI_Strings::get('LABEL_PASSWORD'), "id" => "rm_reg_form_pw_".$this->form_id."_".$this->form_number, "longDesc" => RM_UI_Strings::get('HELP_PASSWORD_MIN_LENGTH'), "minlength" => 7,"labelStyle" => $this->form_options->style_label, "style" => $this->form_options->style_textfield, "validation" => new Validation_RegExp("/.{7,}/", "Error: The %element% must be atleast 7 characters long."))));
