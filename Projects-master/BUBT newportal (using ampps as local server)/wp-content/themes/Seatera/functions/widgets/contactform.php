...
class vh_contactform extends WP_Widget {
	function vh_contactform() {
		...
		parent::__construct('vh_contactform', __('Seatera - Contact Form', 'vh') , $widget_options);
	}
	...
	function widget($args, $instance) {
		...
		<form class="content-form <?php echo $form_class; ?>" action="#" name="contact_form" id="contact_form" method="post">
			...
			<div>
				<input type="text" required="required" id="contact_name" name="contact_name" class="text_input input-block-level" size="22" tabindex="5" onclick="clearInput(this, 'Name (required)');" value="<?php _e('Name (required)', 'vh'); ?>" />
			</div>
			<div>
				<input type="email" required="required" id="contact_email" name="contact_email" class="text_input input-block-level" size="22" tabindex="6" onclick="clearInput(this, 'Email (required)');" value="<?php _e('Email (required)', 'vh'); ?>" />
			</div>
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('email'); ?>"><?php _e('Send emails to:', 'vh'); ?></label>
