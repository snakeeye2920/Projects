...
class vh_advertisement extends WP_Widget {
	...
	public function vh_advertisement() {
		...
		parent::__construct('vh_advertisement', __('Seatera - Advertisement', 'vh'), $widget_opts);
	}
	...
	public function widget($args, $instance) {
		...
		for ($i = 1; $i <= $count; $i++) {
			...
			<div class="advertisement-container">
				<a href="<?php echo $link ?>" rel="nofollow" target="_top" class="no_thickbox" title="<?php _e('Advertisement', 'vh') ?>">
					<img src="<?php echo $image; ?>" alt="<?php _e('Advertisement', 'vh') ?>"<?php echo $class; ?> />
				</a>
			</div>
			<?php
		}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('How many ads to display?', 'vh'); ?></label>
		...
		<div class="advertisement_container">
			...
			for ($i = 1; $i <= $this->max_ads; $i++) {
				...
				<div class="hidden_container" <?php if ($i > $count) { ?>style="display:none"<?php } ?>>
					...
					<p>
						<label for="<?php echo $this->get_field_id($ad_image); ?>"><?php _e('Image URL:', 'vh'); ?></label>
					...
					<p>
						<label for="<?php echo $this->get_field_id($ad_link); ?>"><?php _e('Link:', 'vh'); ?></label>
