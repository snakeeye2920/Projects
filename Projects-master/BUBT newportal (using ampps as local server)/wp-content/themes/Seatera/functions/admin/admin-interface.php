...
function themeoptions_add_admin() {
	...
	$of_page = add_theme_page('Theme Options', 'Theme Options', 'edit_theme_options', 'themeoptions', 'themeoptions_options_page');
...
?>
	<div class="wrap" id="vh_wrapper">
		...
		<form action="" enctype="multipart/form-data" id="ofform">
			...
			<?php
				$theme_options = vh_theme_options($options);
			?>
			<div id="main">
				<div id="of-nav">
					<ul>
...
<?php
	...
	function of_style_only() {
		...
		wp_enqueue_style('jquery-ui', get_template_directory_uri() . '/functions/admin/jquery-ui.css');
...
function ajax_callback() {
	...
	if ($save_type == 'upload') {
		...
		foreach ($options as $option_array) {
			...
			if (isset($option_array['id'])) {
				...
				// ..fonts
					...
					$fonts_content[] = $output[$id . '_font_face'] . ':100,300,400,700';;
		...
		$fonts_content = '@import url(\'http://fonts.googleapis.com/css?family=' . implode('|', $fonts_content) . '&subset=cyrillic,greek,latin\');';
...
function vh_theme_options($options) {
	global $vh_fonts_default_options;
	...
	foreach ($options as $value) {
		...
		if ( !isset( $value['dont_hide'] ) ) {
			$value['dont_hide'] = '';
		}
		if ( !isset( $value['no_id'] ) ) {
			$value['no_id'] = '';
		}
		...
		switch ($value['type']) {
			...
			case 'group_select':
				...
				foreach ($value['options'] as $option) {
					...
					if ($select_value != '') {
						if (str_replace(array('.', ' '), '', strtolower(stripslashes($select_value))) == str_replace(array('.', ' '), '', strtolower(stripslashes($option)))) {
							$selected = ' selected="selected"';
							$selected_option = $option;
						}
					} else {
					...
					$output .= '<option' . $selected . ' value="' . str_replace(array('.', ' '), '', strtolower(stripslashes($option))) . '">';
					$output .= $option;
					$output .= '</option>';
				}
				$output .= '</select>';
			...
			case 'font':
				$color     = $value['color'];
				$option_id = strtolower($value['id']);

				if ( !isset($vh_fonts_default_options[$option_id . '_font_color']) )
					$vh_fonts_default_options[$option_id . '_font_color'] = '';

				if ( !isset($vh_fonts_default_options[$option_id . '_weight']) )
					$vh_fonts_default_options[$option_id . '_weight'] = '';

				$current_face   = get_option($option_id . '_font_face', $vh_fonts_default_options[$option_id . '_font_face']);
				$current_size   = get_option($option_id . '_font_size', $vh_fonts_default_options[$option_id . '_font_size']);
				$current_lh     = get_option($option_id . '_line_height', $vh_fonts_default_options[$option_id . '_line_height']);
				$current_color  = get_option($option_id . '_font_color', $vh_fonts_default_options[$option_id . '_font_color']);
				$current_weight = get_option($option_id . '_weight', $vh_fonts_default_options[$option_id . '_weight']);
				$font_bg        = $vh_fonts_default_options[$option_id . '_bg'];
				...
				$url = wp_nonce_url('themes.php?page=themeoptions','themeoptions');
				...
				$fonts_list = file_get_contents(VH_ADMIN . '/fonts.xml');
				...
				if ($current_weight == '100') {
					$font_options = ' font-weight: 100;';
				} elseif ($current_weight == '300') {
					$font_options = ' font-weight: 300;';
				} elseif ($current_weight == 'bold') {
				...
				$output .= '
					...
					<select name="' . $option_id . '_weight" class="font-weight">
						<option value="100" ' . selected('100', $current_weight, false) . '>100</option>
						<option value="300" ' . selected('300', $current_weight, false) . '>300</option>
					...
					<input style="visibility: hidden; height: 1px; padding: 0; margin: 0;" name="' . $option_id . '_font_color" id="' . strtolower($value['id']) . '" type="text" value="' . $color . '" />
					<div class="clear"></div>
					...
					<div class="font-preview"
						...
						if ( $vh_fonts_default_options[$option_id . '_line_height'] >= 50 ) {
							$output .= 'Marshmallow sugar plum jelly-o pastry.';
						} else {
							$output .= 'Marshmallow sugar plum jelly-o. Pastry chupa chups dragee souffle toffee powder lemon drops
								oat cake. Sweet roll apple pie dragee halvah faworki chupa chups. Topping croissant wafer halvah donut gummi bears
								ice cream jelly beans chupa chups. Biscuit tiramisu danish candy canes lemon drops biscuit. Macaroon fruitcake
								jujubes caramels. Cookie carrot cake gummies biscuit biscuit brownie dragee. Pie candy canes cupcake lemon drops
								chocolate bar.';
						}
			...
			case "group_open":
				$group_class = '';
				$group_id    = '';
				$group_class = str_replace(array('.', ' '), '', strtolower($value['group_class']));
