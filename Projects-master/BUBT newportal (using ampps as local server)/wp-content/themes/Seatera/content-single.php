...
$entry_utility_bottom = '<div class="entry-bottom-utility">';
	if ( 'post' == get_post_type() ) {
		...
		$entry_utility_bottom .= '<div class="blog_time icon-clock">'.get_the_date('j.n.Y').'</div>';
		...
		if ( $tags_list ) {
			...
			<i class="entypo_icon icon-tag-2"></i>
		...
		<div class="open_blog_social">
			...
			<div class="g-plusone" data-size="medium" data-href="' . get_post()->guid . '"></div>
				...
				<?php if ( get_post_type( $post ) == 'post' ) { ?>
					...
					$tc = wp_count_comments($post_id); 
						...
						if ( function_exists('get_post_ul_meta') ) {
							echo '<span class="blog_likes">' . vh_ldc_like_counter_p('') . '</span>';
						}
					?>
					</div>
				<?php } ?>
