...
<!--<![endif]-->
	...
	<body <?php body_class($vh_class); ?>>
		<?php if ( $_SERVER['SERVER_NAME'] == 'cohhe.com' ) { ?>
			<a href="http://themeforest.net/item/seatera-seat-reservation-wordpress-theme/8407389" target="_blank" id="buy-now-ribbon"></a>
		<?php } ?>
			...
			<div class="main">
				...
				<header class="header vc_row wpb_row vc_row-fluid">
					<div class="top-header vc_col-sm-12">
						...
						if ( !empty($menu_header_flickr_url) ) {
							$social_icon_count++;
						}
						...
						if ( $social_icon_count > 0 ) {
							$social_width = $social_icon_count*70;
							$each_icon_width = 100/$social_icon_count;
							$icon_container = ' style="width: '.$social_width.'px;"';
							$icon_width = ' style="width: '.$each_icon_width.'%;"';
						}
						?>
						<div class="header-social-icons vc_col-sm-3"<?php echo $icon_container; ?>>
						<?php if (!empty($menu_header_twitter_url)) { ?>
							<div class="header-icon twitter-icon hover_right"<?php echo $icon_width; ?>><a href="<?php echo $menu_header_twitter_url; ?>" class="icon-twitter-1"></a></div>
						<?php } ?>
						<?php if (!empty($menu_header_flickr_url)) { ?>
							<div class="header-icon flickr-icon hover_right"<?php echo $icon_width; ?>><a href="<?php echo $menu_header_flickr_url; ?>" class="icon-flickr"></a></div>
						<?php } ?>
						<?php if (!empty($menu_header_facebook_url)) { ?>
							<div class="header-icon facebook-icon hover_right"<?php echo $icon_width; ?>><a href="<?php echo $menu_header_facebook_url; ?>" class="icon-facebook"></a></div>
						<?php } ?>
						<?php if (!empty($menu_header_google_url)) { ?>
							<div class="header-icon google-icon hover_right"<?php echo $icon_width; ?>><a href="<?php echo $menu_header_google_url; ?>" class="icon-gplus"></a></div>
						<?php } ?>
						<?php if (!empty($menu_header_pinterest_url)) { ?>
							<div class="header-icon pinterest-icon hover_right"<?php echo $icon_width; ?>><a href="<?php echo $menu_header_pinterest_url; ?>" class="icon-pinterest"></a></div>
						<?php } ?>
					</div>
