...
function vh_search_meta_data_join($join) {
	...
	if ( !in_array('movies', get_query_var( 'post_type' ) ) ) {
		return $join;
	}
...
function vh_search_meta_data_where($where) {
	...
	$where = $startOfQuery . "(" . $wpdb->postmeta . ".meta_value LIKE '%" . get_query_var( 's' ) . "%') OR " . $restOfQuery ." GROUP BY " . $wpdb->posts . ".id";
...
if ( function_exists('add_image_size')) {
	...
	add_image_size('spotlight', 165, 260, true); // spotlight image size
	add_image_size('in_theatres', 100, 155, true); // showing in theatres image size
	add_image_size('offer', 262, 155, true); // offer image size
...
if (!function_exists('vh_scripts_method')) {
	function vh_scripts_method() {
		...
		wp_enqueue_script('jquery.smoothdivscroll', get_template_directory_uri() . '/js/smoothscroll/jquery.smoothDivScroll-1.3.js', array('jquery'), '', TRUE);
		...
		wp_enqueue_script('jquery.dotdotdot', get_template_directory_uri() . '/js/jquery.dotdotdot.min.js', array('jquery'), '', TRUE);
		wp_enqueue_script('jquery.dropdown', get_template_directory_uri() . '/js/modernizr.custom.79639.js', array('jquery'), '', TRUE);
		wp_enqueue_script('jquery.easing', get_template_directory_uri() . '/js/jquery.easing.1.3.min.js', array('jquery'), '', TRUE);
...
if (!function_exists('vh_style_method')) {
	function vh_style_method() {
		...
		wp_enqueue_style('jquery-ui-dialog', get_template_directory_uri() . '/css/jquery-ui-1.10.4.custom.min.css');
...
function prefix_load_cat_posts() {
	...
	if( $the_query->have_posts() ) {
						...
						if ( $theatre == '' ) {
							...
							foreach ( $info_time as $info_time_value ) {
								if ( ( $time != '' && strtotime($info_time_value) >= strtotime($time) && $show_all != '1' ) || $show_all == '1' ) {
									$output .= '<li class="vc_col-sm-4" id="post-'. get_the_ID() .'">';
											...
											$output .= '<div class="inner_top_container">';
												$output .= '<div class="movie_title"><a href="' . get_permalink( get_the_ID() ) . '">' . get_the_title() . '</a></div>';
														if(function_exists('the_ratings')) {
															$rating_images = the_ratings_results($post->ID);
															// $rating_value = number_format(get_post_meta($post->ID, 'ratings_average', true), 1);
															// if ( $rating_value < 10 ) {
															// 	$rating_value = ' ' . $rating_value;
															// }
															$output .= '<div class="event_list_rating">' . $rating_images  . '</div>';
														}
											...
											$output .= '<div class="inner_bottom_container">';
												$output .= '<div class="movie_time icon-clock">';
													...
													foreach ( $info_time as $info_time_values ) {
														if ( ( strtotime($info_time_values) >= strtotime($now) && $show_all != '1' ) || $show_all == '1' || strtotime($current_date["2"]."-".$current_date["0"]."-".$current_date["1"]) > strtotime(date('d-m-Y')) ) {
		...
		$output .= '<li class="no-event-found vc_col-sm-12" id="post-0">';
		$output .= __('There\'s nothing to display.', 'vh');
		$output .= '<li>';
	}
	...
	foreach ($values_arr as $value) {
		...
		if ( (( strtotime($movie_date["2"]."-".$movie_date["0"]."-".$movie_date["1"]) == strtotime($current_date["2"]."-".$current_date["0"]."-".$current_date["1"]) && $show_all != '1' ) || ( strtotime($movie_date["2"]."-".$movie_date["0"]."-".$movie_date["1"]) >= strtotime($current_date["2"]."-".$current_date["0"]."-".$current_date["1"]) && $show_all == '1' )) ) {
			...
			foreach ($combined as $combined_value) {
				...
				$new_button_link = '';
				if ( $button_link == '' ) {
					$new_button_link = get_permalink( $post_id ) . $url_symbol . 'event_id='.$needed_info[2].'&movie='.$post_id;
				} else {
					$new_button_link = $button_link;
				}
...
function vh_load_movies_list() {
	...
	$current_date = explode(".", $post_date);
	$total_movies = $displayed_movies = 0;
	$movies_to_display = array();
	...
	if ( $current_page == 1 ) {
		...
		$paged = $current_page*$per_page-$per_page;
	}

	$paged_till = $current_page*$per_page;
	...
	if( $the_query->have_posts() ) {
		...
		if ( !empty($movies_to_display) ) {
			foreach ($movies_to_display as $movie_value) {
				if ( $total_movies >= $paged && $total_movies < $paged_till ) {
					$output .= '<li class="event_list vc_col-sm-12" id="post-'. $movie_value .'">';
						$output .= '<div class="movie_list_image shadows">';
						$output .= '<div class="bottom_line"></div>';
						if ( kd_mfi_get_featured_image_id( 'event-poster', 'movies', $movie_value ) != '' ) {
							$attachment_id = kd_mfi_get_featured_image_id( 'event-poster', 'movies', $movie_value );
							$image = wp_get_attachment_image_src( $attachment_id, 'movie_list' );
							$output .= '<a href="' . get_permalink( $movie_value ) . '"><img src="'. $image[0] .'"></a>'; 
						} elseif ( has_post_thumbnail( $movie_value ) ) {
							$image = wp_get_attachment_image_src( get_post_thumbnail_id( $movie_value ), 'movie_list' );
							$output .= '<a href="' . get_permalink( $movie_value ) . '"><img src="'. $image[0] .'"></a>'; 
						}
						$output .= '</div>';
						$output .= '<div class="movie_list_container">';
						$output .= '<div class="movie_title"><a href="' . get_permalink( $movie_value ) . '">' . get_the_title( $movie_value ) . '</a></div>';
										if(function_exists('the_ratings')) {
											$rating_images = the_ratings_results($movie_value);
											// $rating_value = number_format(get_post_meta($movie_value, 'ratings_average', true), 1);
											// if ( $rating_value < 10 ) {
											// 	$rating_value = ' ' . $rating_value;
											// }
											$output .= '<div class="event_list_rating">' . $rating_images . '</div>';
										}
										$tc = wp_count_comments($movie_value); 
							...
							$output .= '<div class="overview-container">
								...
								$categories = wp_get_post_terms( $movie_value, 'event_categories',array("fields" => "names") );
								...
								if ( get_post_meta( $movie_value, 'event_release', true ) != '' ) {
									$output .= '<div class="event_list_item icon-calendar"><div class="title left">'.__('Release date:','vh').'</div><div class="info event_release">' . get_post_meta( $movie_value, 'event_release', true ) . '</div><div class="clearfix"></div></div>';
								}
								if ( get_post_meta( $movie_value, 'movies_length', true ) != '' ) {
									$output .= '<div class="event_list_item icon-clock"><div class="title left">'.__('Duration:','vh').'</div><div class="info movies_length">' . vh_convertToHoursMins(get_post_meta( $movie_value, 'movies_length', true ), __('%2d hours %2d minutes', "vh" )) . '</div><div class="clearfix"></div></div>';
								}

							$output .= '</div>
							<div class="main_side_right">';
									if ( get_post_meta( $movie_value, 'event_director', true ) != '' ) {
										$output .= '<div class="event_list_item icon-user"><div class="title right">'.__('Director:','vh').'</div><div class="info event_director">' . get_post_meta( $movie_value, 'event_director', true ) . '</div><div class="clearfix"></div></div>';
									}
									if ( get_post_meta( $movie_value, 'event_actors', true ) != '' ) {
										$output .= '<div class="event_list_item icon-users"><div class="title right">'.__('Actors:','vh').'</div><div class="info event_actors">' . get_post_meta( $movie_value, 'event_actors', true ) . '</div><div class="clearfix"></div></div>';
									}
									if ( get_post_meta( $movie_value, 'event_box_office', true ) != '' ) {
										$output .= '<div class="event_list_item icon-dollar"><div class="title right">'.__('Box Office:','vh').'</div><div class="info event_box_office">' . get_post_meta( $movie_value, 'event_box_office', true ) . '</div><div class="clearfix"></div></div>';
									}
							...
							<div class="clearfix"></div>
									...
									$output .= '<div class="button_red"><a href="'.get_permalink($movie_value).'#tickets'.'" class="vh_button red icon-ticket hover_right">'. __('Buy tickets','vh').'</a></div>';
									$youtube = explode('=',get_post_meta( $movie_value, 'event_trailer', true ));
									if ( $youtube[0] != '' ) {
										if ( $youtube[0] != '' ) {
			...
			$output .= '<li class="event_list vc_col-sm-12" id="post-0">';
			$output .= __('There\'s nothing to display.', 'vh');
			$output .= '<li>';
		}
		$output .= '</ul>';
		...
		if ($cat_id == '' && $theatre == '' ) {
			$page_count = ceil($total_movies/$per_page);
		} else {
			$page_count = ceil($displayed_movies/$per_page);
		}
		...
		if ( $page_count != 1 ) {
			if ( $page_count <= 5 ) {
				for ($i=1; $i <= $page_count; $i++) {
					...
					$output .= '<a href="javascript:void(0)" class="movies_list_pagenum' . $extra . '">' . $i . '</a>';
				}
			} else {
				$output .= vh_get_pagination( intval($current_page), $page_count );
			}
		}
	}
...
function vh_get_pagination( $current_page, $max_pages ) {
	$pages = array();
	$output = '';

	if ( $max_pages > 5 && $current_page > 1 ) {
		$output .= '<a href="javascript:void(0)" class="movies_list_pagenum prev">' . __('<', 'vh') . '</a>';;
	}
	...
	foreach ($pages as $page_value) {
		...
		$output .= '<a href="javascript:void(0)" class="movies_list_pagenum' . $extra . '">' . $page_value . '</a>';
	}

	if ( $max_pages > 5 && $current_page != $max_pages ) {
		$output .= '<a href="javascript:void(0)" class="movies_list_pagenum next">' . __('>', 'vh') . '</a>';;
	}
	...
	if ( $post_image != '' ) {
		...
		<div class="post-thumb">
			<div class="post_info_img">';
				...
				if ( function_exists('get_post_ul_meta') ) {
					$output .= '<span class="blog_likes icon-heart">' . get_post_ul_meta($post_id,"like") . '</span>';
				}
			
			$output .= '</div>
			<div class="post-thumb-img-wrapper shadows">
				<div class="bottom_line"></div>
				<a href="'.get_permalink($post_id).'" class="link_image" title="Permalink to '.get_the_title($post_id).'">
					'.$post_image.'
				</a>
			</div>
		</div>';
		...
		<div class="post-title_nothumbnail">
			<div class="post_info_text">
				<span class="comments icon-comment">'.$tc->total_comments.'</span>';
				if ( function_exists('get_post_ul_meta') ) {
					$output .= '<span class="blog_likes icon-heart">' . get_post_ul_meta($post_id,"like") . '</span>';
				}
			$output .= '
...
function vh_post_bottom_module( $value, $data ) {
	...
	$output .= '<div class="read_more">
					<a href="http://localhost/seatera/transcendence/" class="vc_read_more hover_right" title="Permalink to Transcendence in a world">Read more</a>
				</div>
				<div class="blog_author">
					<div class="blog_time icon-clock">'.get_the_date('j.n.Y', $post_id).'</div>
					<span class="author icon-user"><a href="'.get_author_posts_url( get_post_field( 'post_author', $post_id ) ).'">'.__('by', 'vh').' '.get_userdata( get_post_field( 'post_author', $post->id ) )->display_name.'</a></span>
				</div>';
...
function admin_movies_styles(){
	...
	if(  is_admin() ) {
		...
		wp_enqueue_style('tags', get_template_directory_uri() . '/css/jquery.tagit.css');
		wp_enqueue_style('jquery-ui-dialog', get_template_directory_uri() . '/css/jquery-ui-1.10.4.custom.min.css');
		wp_enqueue_style('admin-dialog', get_template_directory_uri() . '/css/admin-dialog.css');
	}
}
...
function vh_register_required_plugins() {
	...
	$plugins = array(
		array(
			...
			'source'   				=> get_template_directory() . '/functions/tgm-activation/plugins/seatera-plugin.zip', // The plugin source
		...
		array(
			...
			'required'              => false, // If false, the plugin is only 'recommended' instead of required
			'version'               => '1.82', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation'      => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation'    => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url'          => '', // If set, overrides default API URL and points to an external URL
		),
		array(
			...
			'source'                => get_template_directory() . '/functions/tgm-activation/plugins/row-seats-core.zip', // The plugin source
			'required'              => true, // If false, the plugin is only 'recommended' instead of required
			'version'               => '2.54', // E.g. 1.0.0. If set, the active plugin must be this version or higher, otherwise a notice is presented
			'force_activation'      => true, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch
			'force_deactivation'    => true, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins
			'external_url'          => '', // If set, overrides default API URL and points to an external URL
		),
